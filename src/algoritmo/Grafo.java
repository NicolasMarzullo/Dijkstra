...
public class Grafo {
	...
	public int[] calcularDijkstra(int nodoSolicitado) {
		...
		while (!this.colaCostoArista.isEmpty()) {
			...
			// Cargo los adyacentes. Sé que un nodo no es adyacente con otro si tiene un
			// infinito.
			for (int i = 0; i < this.cantidadDeNodos; i++) {
			...
			for (Integer i : listaDeAdyacentes) {
				...
				// Calculo los costos yendo directamente al nodo o pasando por un intermedio a
				// ver cual es mejor
				...
				if (costoConNodoIntermedio < costoDirecto) {
					...
					// Esto es para actualizar la cola, ya que solo se puede actualizar eliminando y
					// volviendo a agregar el elemento
